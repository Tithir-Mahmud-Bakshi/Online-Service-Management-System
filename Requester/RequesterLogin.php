...
if(!isset($_SESSION['is_login'])){
 ...
 {
	$rEmail = mysqli_real_escape_string($connection, trim($_REQUEST['rEmail'])) ;
	$rPassword = mysqli_real_escape_string($connection, trim($_REQUEST['rPassword']));
	$sql = "SELECT r_email, r_password FROM requesterlogin_tb WHERE r_email = '".$rEmail."' AND r_password = '".$rPassword."' limit 1 ";
	$result = $connection->query($sql);
	if($result->num_rows == 1){
		$_SESSION['is_login'] = true;
		$_SESSION['rEmail'] = $aEmail; 
